...
module('Integration | Component | hello', function (hooks) {
  ...
  test('it renders (4)', async function (this: TestContext, assert) {
    await render<TestContext>(
      // prettier-ignore
      hbs`<Hello @age={{30}} @name={{this.name}} data-test-message />`,
    );
  });
  ...
  test('it renders (6)',
    async function (this: TestContext, assert) {
      await render<TestContext>(
        hbs`<Hello @age={{30}} {{! prettier-ignore }} @name={{concat "Zoey" this.someSeparator "von Ember"}} data-test-message="" />`,
      );
    },
  );
  ...
  test('it renders (7)', async function (this: TestContext, assert) {
    await render<TestContext>(
      hbs`
        {{! prettier-ignore }}
        <Hello @age={{30}} {{! @glint-expect-error }} @name={{hash first="Zoey" last="von Ember"}} data-test-message />
      `,
    );
  });
  ...
  test('it renders (9)', async function (this: TestContext, assert) {
    // prettier-ignore
    await render<TestContext>(hbs`
      <Aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa @name={{this.name}} data-test-message @age={{30}} />
    `);
  });
});
